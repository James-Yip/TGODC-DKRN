...
# SAVE_CUR_UT_FILE = 'new_source.txt'
...
def convert_context_with_bert_tokenizer():
    ...
    tokenizer = tokenization.FullTokenizer(
        vocab_file='../tx_weibo_data/bertvocab.txt',
        do_lower_case=True)
    for stage in ['train','valid','test']:
        with open(os.path.join('../tx_weibo_data', stage, 'context.txt'), 'r') as f:
            original_context_list = f.readlines()
        bert_context_list = tokenize_sequences(original_context_list, tokenizer)
        with open(os.path.join('../tx_weibo_data', stage, 'context_bert.txt'), 'w') as f:
...
def obtain_utterance_transitions_list(histories_strs, target_strs):
    ...
    cur_utterances = [histories_str.replace('|||',' ||| ') for histories_str in histories_strs]
...
# for stage in ['train','valid','test']:
#     with open(os.path.join('../tx_weibo_data', stage, SAVE_KW_TRANS_FILE), 'w') as f:
...
#     with open(os.path.join('../tx_weibo_data', stage, SAVE_KW_TRANS_LABEL_FILE), 'w') as f:
#         for _ in range(len(keywords_transitions_dict[stage])):
#             f.write('1\n0\n')

# for stage in ['train','valid','test']:
#     with open(os.path.join('../tx_weibo_data', stage, SAVE_KW_TRANS_FILE), 'w') as f:
#         with open(os.path.join('../tx_weibo_data', stage, SAVE_KW_TRANS_LABEL_FILE), 'w') as labelfile:
#             with open(os.path.join('../tx_weibo_data', stage, SAVE_KW_TRANS_SEG_FILE), 'w') as segfile:
#                 for cur_keyword, right_next_keyword, wrong_next_keyword in keywords_transitions_dict[stage]:
...
def generate_kwts_dataset():
    ...
    keywords_dict = {
        stage: load_file(os.path.join('../tx_weibo_data', stage, 'keywords.txt'))
        for stage in ['train','valid','test']
    }
    # load conversation history files of train, valid and test sets
    histories_dict = {
        stage: load_file(os.path.join('../tx_weibo_data', stage, 'source.txt'))
        for stage in ['train','valid','test']
    }
...
def generate_kwts_dataset_v2():
    ...
    for stage in ['train','valid','test']:
        with open(os.path.join('../tx_weibo_data', stage, SAVE_CUR_KW_FILE), 'w') as cur_kw_f:
            with open(os.path.join('../tx_weibo_data', stage, SAVE_NEXT_KW_FILE), 'w') as next_kw_f:
                for cur_keyword, right_next_keyword, wrong_next_keyword in keywords_transitions_dict[stage]:
...
def generate_utts_dataset():
    ...
    targets_dict = {
        stage: load_file(os.path.join('../tx_weibo_data', stage, 'target.txt'))
        for stage in ['train','valid','test']
    }
    ...
    for stage in ['train','valid','test']:
        with open(os.path.join('../tx_weibo_data', stage, SAVE_CUR_UT_FILE), 'w') as cur_utter_f:
            with open(os.path.join('../tx_weibo_data', stage, SAVE_NEXT_UT_FILE), 'w') as next_utter_f:
        ...
        with open(os.path.join('../tx_weibo_data', stage, SAVE_UT_TRANS_LABEL_FILE), 'w') as f:
...
def generate_target_guide_train_dataset():
    ...
    context_dict = {
        stage: load_file(os.path.join('../tx_weibo_data', stage, 'context.txt'))
        for stage in ['train','valid','test']
    }
    # load keyword files of train, valid and test sets
    keywords_dict = {
    ...
    for stage in ['train','valid','test']:
        with open(os.path.join('../tx_weibo_data', stage, SAVE_TARGET_GUIDED_CONTEXT_FILE), 'w') as tg_context_f:
            with open(os.path.join('../tx_weibo_data', stage, SAVE_TARGET_GUIDED_KEYWORDS_FILE), 'w') as tg_keyword_f:
                for dialogue_context_keywords_list in dialogue_context_keywords_dict[stage]:
                    ...
                    for keyword in last_keywords:
                        ...
                        for context_word in last_context:
                            total_sim += get_similarity(keyword, context_word,w2v_model)
...
def generate_target_guide_single_turn_train_dataset():
    ...
    for stage in ['train','valid','test']:
        with open(os.path.join('../tx_weibo_data', stage, SAVE_TARGET_GUIDED_CONTEXT_ST_FILE), 'w') as tg_context_f:
            with open(os.path.join('../tx_weibo_data', stage, SAVE_TARGET_GUIDED_KEYWORDS_ST_FILE), 'w') as tg_keyword_f:
                for dialogue_context_keywords_list in dialogue_context_keywords_dict[stage]:
                    last_context_keywords = dialogue_context_keywords_list[-1]
                    last_context = last_context_keywords[0]
                    last_keywords = last_context_keywords[1]
...
def create_chinese_glove_corpus():
    corpus_lines = load_file(os.path.join('../tx_weibo_data', 'corpus.txt'))
    start_corpus_lines = load_file(os.path.join('../tx_weibo_data', 'start_corpus.txt'))
    with open('../tx_weibo_data/glove_corpus.txt', 'w') as glove_f:
        for line in corpus_lines:
            glove_f.write(line + '\n')

        for line in start_corpus_lines:
            glove_f.write(line + '\n')


def generate_target_keywords_for_simulation():
    keywords_file = '../tx_weibo_data/test/keywords_vocab.txt'
    target_file = '../tx_weibo_data/target_keywords_for_simulation.txt'
...
def generate_start_utterances_for_simulation():
    start_corpus_file = '../tx_weibo_data/start_corpus.txt'
    target_corpus_file = '../tx_weibo_data/sample_start_corpus.txt'
